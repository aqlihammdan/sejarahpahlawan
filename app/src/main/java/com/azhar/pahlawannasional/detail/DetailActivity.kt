...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnshare.setOnClickListener{
            val modelMain = intent.getSerializableExtra(DETAIL_PAHLAWAN) as ModelMain

            val intent = Intent().apply {
                action = Intent.ACTION_SEND
                putExtra(Intent.EXTRA_TEXT, modelMain.namaLengkap)
                type = "text/plain"
            }
            val shareIntent = Intent.createChooser(intent, "Select App:")
